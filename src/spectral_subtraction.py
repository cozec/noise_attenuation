"""Standalone spectral subtraction for noise attenuation.

Self-contained duplicate of the spectral subtraction section of the Aalto ITSP
book chapter "Noise attenuation"
...
spectral subtraction gain -> ISTFT -> save enhanced audio -> plots.

Spectral subtraction removes the noise estimate in the magnitude domain,
G = sqrt(max(|Y|^2 - N, 0) / |Y|^2), whereas the Wiener filter
(wiener_filtering.py) uses the power-domain ratio without the square root.

Usage:
    python spectral_subtraction.py [input.wav]

Defaults to data/sounds/enhancement_test.wav. Outputs:
    results/spectral_subtraction_enhanced.wav
    plots/spectral_subtraction_waveforms.png
    plots/spectral_subtraction_spectrograms.png
    plots/spectral_subtraction_noise_model.png
"""
...
def spectral_subtraction(spectrogram, noise_estimate):
    """Apply the spectral subtraction gain to a noisy spectrogram.

    The noise power estimate is subtracted from the noisy power spectrum,
    thresholded at zero, and the gain is applied in the magnitude domain:
    G = sqrt(max(|Y|^2 - N, 0) / |Y|^2), per time-frequency bin.
    """
    ...
    gain = np.sqrt(speech_power / noisy_power)
...
def main():
    ...
    # 4. Spectral subtraction gain + 5. reconstruction
    spectrogram_enhanced, gain = spectral_subtraction(spectrogram, noise_estimate)
    data_enhanced = istft(spectrogram_enhanced, fs, window_length_ms, window_step_ms,
                          fft_size=fft_size)

    out_wav = os.path.join(results_dir, 'spectral_subtraction_enhanced.wav')
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('Amplitude')
    plt.title('Spectral-subtraction-enhanced waveform')
    plt.axis([0, t[-1], 1.05 * np.min(data), 1.05 * np.max(data)])
    plt.tight_layout()
    path = os.path.join(plots_dir, 'spectral_subtraction_waveforms.png')
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('Frequency (kHz)')
    plt.title('Spectral-subtraction-enhanced spectrogram')
    plt.tight_layout()
    path = os.path.join(plots_dir, 'spectral_subtraction_spectrograms.png')
    plt.savefig(path, dpi=100, bbox_inches='tight')
    plt.close()
    print(f'wrote {path}')
    ...
    path = os.path.join(plots_dir, 'spectral_subtraction_noise_model.png')
    plt.savefig(path, dpi=100, bbox_inches='tight')
    plt.close()
    print(f'wrote {path}')
